...
int main(void){
    ...
    int choice=1;
    Observer* obs[3]={
            new Under(3), new Over(3), new Status()
    };
    std::string observerName[3]={"Under(3)","Over(3)", "Status()"};
    UndoStack undos;
    ...
    for(int i=0;i<3;i++){
        ...
        std::cout << observerName[i];
        std::cout << ((obs[i]->state())?" true":" false") << std::endl;
    }
    ...
    while(choice != 0){
        if(choice>=1 && choice <=6){
            ...
            double currtime=theTimer->currtime();
            theTimer->notify();
            std::cout << "Current Timer Reading: " << currtime;
            std::cout << ((theTimer->isPaused())?" paused":" unpaused") << std::endl;
            for(int i=0;i<3;i++){
